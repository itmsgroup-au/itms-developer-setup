...
class ProjectContextManager:
    ...
    def __init__(self):
        ...
        self.mcp_configs = {
            ...
            'cursor_kilo': Path.home() / 'Library' / 'Application Support' / 'Cursor' / 'User' / 'globalStorage' / 'kilocode.kilo-code' / 'settings' / 'mcp_settings.json',
            'augment': Path.home() / 'Library' / 'Application Support' / 'Cursor' / 'User' / 'globalStorage' / 'augment.vscode-augment' / 'augment-global-state' / 'mcpServers.json',
        ...
        self.session.headers.update({
            'User-Agent': 'ITMS-Context-Manager/1.0',
    ...
    def select_board(self) -> Optional[Dict]:
        ...
        current_board_id = self.current_context.get('board_id') if self.current_context else None
    ...
    def select_repo(self) -> Optional[Dict]:
        ...
        current_repo = self.current_context.get('repo_full_name') if self.current_context else None
    ...
    def update_odoo_config_file(self, config: Dict):
        ...
        print(f"✅ Updated Odoo MCP config: {odoo_config['odoo_url']} / {odoo_config['odoo_db']}")
        ...
        for config_name, config_path in self.mcp_configs.items():
            try:
                if config_path.exists():
                    self.update_mcp_config_file(config_path, config_name, board_id, repo_full_name, repo_owner)
                    updated_count += 1
                else:
    ...
    def update_mcp_config_file(self, config_path: Path, config_name: str, board_id: str, repo_full_name: str, repo_owner: str):
        ...
        if config_name == 'augment':
            ...
            # Standard MCP format
            self.update_standard_mcp_config(config, board_id, repo_full_name, repo_owner)
    ...
    def update_augment_config(self, config: list, board_id: str, repo_full_name: str, repo_owner: str):
    ...
    def update_standard_mcp_config(self, config: dict, board_id: str, repo_full_name: str, repo_owner: str):
        ...
        servers['postgresql'] = {
            "command": "npx",
            "args": ["-y", "@modelcontextprotocol/server-postgres", "postgresql://postgres@localhost:5432/postgres"],
            "env": {
                "PG_PATH": "/Applications/Postgres.app/Contents/Versions/17/bin"
            }
        }
        ...
        if 'git' not in servers:
            servers['git'] = {
                "command": "uvx",
                "args": ["mcp-server-git", "--repository", os.getenv('PROJECT_ROOT', '/Users/markshaw/Desktop/git')]
            }
            ...
            try:
                ...
                if choice == '0':
                    ...
                    if board and self.current_context:
                        ...
                        self.current_context['updated_at'] = __import__('datetime').datetime.now().isoformat()
