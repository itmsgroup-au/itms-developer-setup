    ...
    def handle_request(self, request):
        ...
        if method == "initialize":
                        ...
                        },
                        {
                            ...
                            "inputSchema": {
                                ...
                                "properties": {
                                    "task_id": {
                                        "type": "string",
                                        "description": "Monday.com task ID",
                                    }
                                },
                                "required": ["task_id"],
                            },
                        ...
                        },
                        {
                            ...
                            "inputSchema": {
                                ...
                                "properties": {
                                    "subtask_id": {
                                        "type": "string",
                                        "description": "Monday.com subtask ID",
                                    }
                                },
                                "required": ["subtask_id"],
                            },
                        ...
                        },
                        {
                            ...
                            "inputSchema": {
                                ...
                                "properties": {
                                    "group_id": {
                                        "type": "string",
                                        "description": "Monday.com group ID",
                                    }
                                },
                                "required": ["group_id"],
                            },
            ...
            if tool_name == "get_monday_tasks":
                ...
                return self.complete_subtask(
                    request,
                    tool_args.get("subtask_id"),
                    tool_args.get("completion_notes", ""),
                )
    ...
    def get_monday_tasks(self, request):
        ...
        if not self.monday_token:
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "No Monday.com token"},
            }
            ...
            if response.status_code == 200:
                ...
                if "errors" in data:
                    return {
                        ...
                        "error": {
                            "code": -1,
                            "message": f"GraphQL errors: {data['errors']}",
                        },
                    }
...
**Recent Updates:**"""
                    ...
                    task_list = "\\n".join(
                        [
                            f"• **{task['name']}** (ID: {task['id']}, Group: {task['group']['title']}, Status: {task['state']}, Updates: {len(task['updates'])})"
                            for task in tasks
                        ]
                    )
                    group_filter_text = ""
                    if hasattr(self, "working_group_name") and self.working_group_name:
                        group_filter_text = (
                            f" (filtered to group: {self.working_group_name})"
                        )
                    return {
                        "jsonrpc": "2.0",
                        "id": request.get("id"),
    ...
    def create_monday_task(self, request):
        ...
        if not self.monday_token or not name:
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "Missing token or name"},
            }
                ...
                return {
                    ...
                    "result": {
                        "content": [
                            {
                                "type": "text",
                                "text": f"Created task: {name} (ID: {result['data']['create_item']['id']})",
                            }
                        ]
                    },
                }
    ...
    def select_active_task(self, request, task_id):
        ...
        if not self.monday_token or not task_id:
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "Missing token or task_id"},
            }
            ...
            if response.status_code == 200:
                ...
                if not data["data"]["items"]:
                    return {
                        "jsonrpc": "2.0",
                        "id": request.get("id"),
                        "error": {"code": -1, "message": "Task not found"},
                    }
...
**Recent Updates:**"""
                ...
                return {
                    "jsonrpc": "2.0",
                    "id": request.get("id"),
                    "result": {"content": [{"type": "text", "text": task_details}]},
                }
            else:
                return {
                    "jsonrpc": "2.0",
    ...
    def create_subtasks(self, request):
        ...
        if not active_task:
            return {
                ...
                "error": {
                    "code": -1,
                    "message": "No active task selected. Use get_monday_tasks or select_active_task first.",
                },
            }
        ...
        for subtask_name in subtasks:
            ...
            try:
                ...
                if response.status_code == 200:
                    result = response.json()
                    if "errors" not in result and result.get("data", {}).get(
                        "create_subitem"
                    ):
        ...
        return {
            ...
            "result": {
                "content": [
                    {
                        "type": "text",
                        "text": f"Created {len(created_subtasks)} sub-tasks for: {active_task['name']}\\n"
    ...
    def execute_subtask(self, request, subtask_id):
        ...
        if not subtask_id:
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "Missing subtask_id"},
            }
    ...
    def complete_subtask(self, request, subtask_id, completion_notes=""):
        ...
        if not self.monday_token or not subtask_id:
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "Missing token or subtask_id"},
            }
        ...
        try:
                    ...
                    return {
                        ...
                        "result": {
                            "content": [
                                {
                                    "type": "text",
                                    "text": f"Completed subtask {subtask_id}\\nNotes: {completion_notes}",
                                }
                            ]
                        },
                    }
            ...
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "Failed to update subtask status"},
            }
    ...
    def set_working_board(self, request, board_id):
        ...
        if not board_id:
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "Missing board_id"},
            }
    ...
    def set_working_repo(self, request, repo):
        ...
        if not repo:
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "Missing repo"},
            }
        ...
        return {
            ...
            "result": {
                "content": [
                    {
                        "type": "text",
                        "text": f"Set working GitHub repository to: {repo}",
                    }
                ]
            },
        }
            ...
            if response.status_code == 200:
                ...
                if "errors" not in data and data["data"]["boards"]:
                    ...
                    if active_groups:
                        group_list = "\\n".join(
                            [
                                f"• {group['title']} (ID: {group['id']}, Color: {group['color']})"
    ...
    def set_working_group(self, request, group_id):
        ...
        if not group_id:
            return {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": -1, "message": "Missing group_id"},
            }
                ...
                return {
                    ...
                    "result": {
                        "content": [
                            {
                                "type": "text",
                                "text": f"Set working group to: {group_name} (ID: {group_id})\\n\\nTasks will now be filtered to this group only.",
                            }
                        ]
                    },
                }
        ...
        return {
            ...
            "result": {
                "content": [
                    {
                        "type": "text",
                        "text": "Cleared working group filter. Tasks will now show from all groups on the board.",
                    }
                ]
            },
        }
